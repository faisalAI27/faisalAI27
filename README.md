# 👋 Hi, I'm Faisal Imran

[![Typing SVG](https://readme-typing-svg.herokuapp.com?size=24&duration=3000&color=00BFFF&lines=AI%2FML+Engineer+in+Progress;B.S.+Artificial+Intelligence+Student;Building+AI+and+Software+Systems;Learning+MLOps%2C+RAG%2C+LLMs+and+Deep+Learning)](https://git.io/typing-svg)



🎓 **B.S. Artificial Intelligence Student at GIKI**  
🤖 **Aspiring AI/ML Engineer | AI Builder**  
📍 Pakistan  

I am an Artificial Intelligence student passionate about building practical AI-powered systems that solve real-world problems. I enjoy working at the intersection of **Artificial Intelligence and Software Engineering**, where intelligent models are combined with useful applications, APIs, databases, automation workflows, and deployment tools.

My main interests include **Machine Learning, Deep Learning, MLOps, LLMs, RAG, Agentic AI, Generative AI, Bioinformatics AI, Computer Vision, and Full-Stack AI Applications**.

---

## 🧠 About Me

- 🎓 Pursuing **B.S. Artificial Intelligence** at **Ghulam Ishaq Khan Institute of Engineering Sciences and Technology**
- 💻 Interested in **AI/ML Engineering, Deep Learning, and real-world AI product development**
- 🔎 Exploring **LLMs, RAG systems, MLOps, Bioinformatics AI, Computer Vision, and Automation**
- 🛠️ Experienced with both **AI model development** and **full-stack application development**
- 🚀 Focused on improving my skills through hands-on projects, internships, research, and continuous learning

---

## 🛠️ Skills & Technologies

### Programming Languages
`Python` `C/C++` `SQL` `JavaScript` `TypeScript` `Bash` `HTML` `CSS`

### AI / Machine Learning
`Machine Learning` `Deep Learning` `MLOps` `LLMs` `Generative AI` `RAG` `Agentic AI`  
`Prompt Engineering` `Time Series Forecasting` `Bioinformatics AI` `Computer Vision`

### Frameworks & Libraries
`PyTorch` `TensorFlow` `Hugging Face Transformers` `Scikit-learn` `OpenCV`  
`FastAPI` `Flask` `React` `Next.js` `Streamlit` `Gradio` `Node.js` `Express.js`

### Tools & Infrastructure
`Git` `GitHub` `Docker` `Docker Compose` `GitHub Actions` `CI/CD` `Linux`  
`AWS EC2` `AWS ECR` `GHCR` `Nginx` `PostgreSQL` `ChromaDB` `Vector Databases`  
`Playwright` `BeautifulSoup` `Prefect` `Pytest` `REST APIs` `WebSockets`

---

---

---


## 🎓 Education

**Ghulam Ishaq Khan Institute of Engineering Sciences and Technology**  
**B.S. Artificial Intelligence**  
📍 Topi, Pakistan  
📅 2023 – 2027

---



## 📫 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Faisal%20Imran-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/faisal-imran-644088327/)

[![GitHub](https://img.shields.io/badge/GitHub-faisalAI27-black?style=for-the-badge&logo=github)](https://github.com/faisalAI27)

[![Email](https://img.shields.io/badge/Email-faisalimran2023189%40gmail.com-red?style=for-the-badge&logo=gmail)](mailto:faisalimran2023189@gmail.com)

---

⭐ *Always learning, building, and improving through Artificial Intelligence and Software Engineering.*
